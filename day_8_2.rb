...
antennas.each do |char, antenna|
  antenna.each do |antenna_1|
    antenna.each do |antenna_2|
      ...
      until outside_of_array?(node_1[0], output.length) || outside_of_array?(node_1[1], output[0].length)
        antinodes.add(node_1)
        node_1 = [node_1[0] + distance_calc[0], node_1[1] + distance_calc[1]]
      end
      until outside_of_array?(node_2[0], output.length) || outside_of_array?(node_2[1], output[0].length)
        antinodes.add(node_2)
        node_2 = [node_2[0] - distance_calc[0], node_2[1] - distance_calc[1]]
      end
    end
  end
end
